    ...
    /// Festlegung, welche Farben zu welchem Zeitpunkt verwendet werden
        sky(): void {
            ...
            if (this.current > 0 && this.current < DayNightCycle.total * 0.02) {
                c.fillStyle = this.colorFade(39, 33, 78, 255, 107, 62, DayNightCycle.time % DayNightCycle.total, DayNightCycle.total * 0.02);
            }
            if (this.current > DayNightCycle.total * 0.02 && this.current < DayNightCycle.total * 0.6) {
                c.fillStyle = this.colorFade(255, 107, 62, 181, 214, 224, DayNightCycle.time % DayNightCycle.total - (DayNightCycle.total * 0.02), DayNightCycle.total * 0.04);
            }
            ...
            if (this.current > DayNightCycle.total * 0.44 && this.current < DayNightCycle.total * 0.47) {
                c.fillStyle = this.colorFade(181, 214, 224, 255, 107, 62, DayNightCycle.time % DayNightCycle.total - (DayNightCycle.total * 0.44), DayNightCycle.total * 0.03);
            }
            if (this.current > DayNightCycle.total * 0.47 && this.current < DayNightCycle.total * 0.5) {
                c.fillStyle = this.colorFade(255, 107, 62, 39, 33, 78, DayNightCycle.time % DayNightCycle.total - (DayNightCycle.total * 0.47), DayNightCycle.total * 0.03);
            }
            if (this.current > DayNightCycle.total * 0.5 && this.current < DayNightCycle.total) {
