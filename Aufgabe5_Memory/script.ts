namespace L03_Memory {
    ...
    const settingsForm: HTMLFormElement = document.querySelector("#settingsForm");
    ...
    const cardStorage: { [key: string]: PartialCard } = {};
    ...
    function placeCards(cards: Card[]): void {
        ...
        for (let card of partialCards) {
            ...
            (cardElement.querySelector(".card-icon-wrapper>svg>path") as HTMLElement).style.fill = card.type.color;
    ...
    function selectAndPlaceCards(): void {
        ...
        const cards = selectCards(parseInt(formData.get("cardCount") as string));

        let rootStyle = (document.querySelector(":root") as HTMLElement).style;

        rootStyle.setProperty("--card-size", formData.get("cardSize") + "px");
        rootStyle.setProperty("--back-color", formData.get("bgColor") as string);
        rootStyle.setProperty("--background-color", formData.get("fBgColor") as string);
        rootStyle.setProperty("--text-color", formData.get("textColor") as string);
        rootStyle.setProperty("--text-font", formData.get("textFont") as string);
